...
def get_speed(body, x, y):
	...
	if now_speed_y > y:
		...
		dir_y = 1
	speed_dis_x = math.sqrt(abs(now_speed_x**2 - x**2)) * dir_x
	speed_dis_y = math.sqrt(abs(now_speed_y**2 - y**2)) * 0.66
...
A2 = np.array([[1.0, 0.05], 
               [0.05, 1.0]])  
Q2 = np.array([[0.05, 0.15], 
               [0.15, 0.05]])	
t2 = np.array([[0.0], 
               [0.0]])  
m2 = np.array([[0.0], 
...
# --- pybox2d world setup ---
# Create the world
world = world(gravity=(0, dir_gravity * 5), doSleep=True)
...
rectangle_body = world.CreateDynamicBody(position=(25, 30), angle= math.radians(-30))
...
box = rectangle_body.CreatePolygonFixture(box=(1.5, 2), density=1, friction=0.3)
start_shape.append(list(rectangle_body.fixtures[0].shape.vertices))

# Create another dynamic body
rectangle_body2 = world.CreateDynamicBody(position=(35, 30), angle= math.radians(0))
velocity1 = (-2.25, 10.0)  # 速度向量的方向和大小
point1 = rectangle_body2.worldCenter
rectangle_body2.ApplyLinearImpulse(velocity1, point1, wake=True)
# And add a box fixture onto it (with a nonzero density, so it will move)
box = rectangle_body2.CreatePolygonFixture(box=(2.5, 2.5), density=1, friction=0.3)
start_shape.append(list(rectangle_body2.fixtures[0].shape.vertices))


# Create another dynamic body
rectangle_body3 = world.CreateDynamicBody(position=(25, 45), angle= math.radians(0))
velocity2 = (-2.0, -5.0)  # 速度向量的方向和大小
point2 = rectangle_body3.worldCenter
rectangle_body3.ApplyLinearImpulse(velocity2, point2, wake=True)
# And add a box fixture onto it (with a nonzero density, so it will move)
box = rectangle_body3.CreatePolygonFixture(box=(2, 2), density=1, friction=0.3)
start_shape.append(list(rectangle_body3.fixtures[0].shape.vertices))
...
last_speed_x = [0.0, 0.0, 0.0]
last_speed_y = [0.0, 0.0, 0.0]
extend = [4, 4, 4]
flag = 3
...
while running:
	...
	screen.fill((0, 0, 0, 0))
	# Draw the world
	for body in (ground_body, rectangle_body, rectangle_body2, rectangle_body3):  # or: world.bodies
		...
		for fixture in body.fixtures:
			...
			if body.type == 2:
				...
				extend_shape = expand_rectangle(start_shape[flag], extend[flag])
